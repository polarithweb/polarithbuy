...
export function BooksAdminPortal({ 
  books, 
  onBack, 
  onSwitchPortal,
  initialEditBook 
}: BooksAdminPortalProps) {
  ...
  // Editing state
  const [editingBookId, setEditingBookId] = useState<string | null>(initialEditBook?.id || null);

  // Form State
  const [title, setTitle] = useState(initialEditBook?.title || '');
  const [type, setType] = useState<string>(initialEditBook?.type || BOOK_TYPES[0]);
  const [author, setAuthor] = useState(initialEditBook?.author || '');
  const [price, setPrice] = useState(initialEditBook?.price || '');
  const [description, setDescription] = useState(initialEditBook?.description || '');
  const [cashOnDeliveryEligible, setCashOnDeliveryEligible] = useState<boolean>(initialEditBook?.cashOnDeliveryEligible ?? true);
  const [imagePreview, setImagePreview] = useState<string | null>(initialEditBook?.imageUrl || null);
  ...
  useEffect(() => {
    if (initialEditBook && isAuthenticated) {
      startEditing(initialEditBook);
    }
  }, [initialEditBook, isAuthenticated]);
  ...
  const startEditing = (book: Book) => {
    ...
    setTitle(book.title);
    setType(book.type || BOOK_TYPES[0]);
    setAuthor(book.author || '');
    setPrice(book.price || '');
    setDescription(book.description || '');
    setCashOnDeliveryEligible(book.cashOnDeliveryEligible !== false);
  ...
  const cancelEditing = () => {
    ...
    setTitle('');
    setType(BOOK_TYPES[0]);
    setAuthor('');
    setPrice('');
    setDescription('');
    setCashOnDeliveryEligible(true);
  ...
  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault();
    if (!title.trim()) {
      setFeedback({ type: 'error', message: 'Please enter a book name.' });
      return;
    }
    ...
    try {
      ...
      if (editingBookId) {
        ...
        await updateBook(editingBookId, {
          title: title.trim(),
          type: type,
          author: author.trim() || 'Polarith Editorial',
          price: formattedPrice,
          description: description.trim(),
          imageUrl: imagePreview || '',
          cashOnDeliveryEligible,
        });
  ...
  const handleDelete = async (bookId: string, bookTitle: string) => {
    ...
    try {
      ...
      setFeedback({ type: 'success', message: `"${bookTitle}" was deleted successfully from Firebase.` });
    } catch (err) {
      ...
      >
        ...
        <div className="w-full p-8 rounded-3xl bg-[#EAF4FF] border border-[#BFDBFE] shadow-sm flex flex-col items-center text-center">
          ...
          <p className="text-xs text-slate-600 mb-6 max-w-xs leading-relaxed">
            This portal is password-protected. Enter the administrator passcode to manage inventory and products.
          </p>
    ...
    >
      ...
      <div ref={formTopRef} className="pt-8 pb-4">
        ...
        <p className="mt-1.5 text-sm text-slate-500">
          Upload new books, edit existing titles, or delete items. Real-time Firebase Firestore database.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mt-6">
        {/* Left Column: Direct Upload & Add/Edit Book Form */}
        <div className="lg:col-span-5 bg-[#EAF4FF] p-6 sm:p-7 rounded-2xl border border-[#BFDBFE] shadow-sm">
          <div className="flex items-center justify-between mb-5 pb-3 border-b border-sky-200/60">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-base font-bold text-slate-900">
                {editingBookId ? 'Edit Product' : 'Upload New Product'}
              </h2>
            </div>
            {editingBookId && (
              <button
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {imagePreview ? (
                <div className="relative group rounded-xl overflow-hidden border border-sky-300 bg-white p-2.5 flex items-center justify-center gap-4">
                  ...
                  <div className="flex-1 text-left">
                    ...
                    <p className="text-xs font-medium text-slate-700">Cover uploaded</p>
                    <button
                      type="button"
            ...
            {/* Book Name */}
            <div>
              <label htmlFor="book-title" className="block text-xs font-semibold text-slate-700 mb-1">
            ...
            {/* Select a Type */}
            <div>
              <label htmlFor="book-type" className="block text-xs font-semibold text-slate-700 mb-1">
                Select a Type <span className="text-rose-500">*</span>
              </label>
              <select
                id="book-type"
                value={type}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label htmlFor="book-author" className="block text-xs font-semibold text-slate-700 mb-1">
                  Author / Publisher
                </label>
                <input
                  id="book-author"
              ...
              <div>
                <label htmlFor="book-price" className="block text-xs font-semibold text-slate-700 mb-1">
                  Price
                </label>
                <input
                  id="book-price"
            ...
            {/* Description */}
            <div>
              <label htmlFor="book-desc" className="block text-xs font-semibold text-slate-700 mb-1">
                Description
              </label>
              <textarea
                id="book-desc"
                ...
                placeholder="Product description or key highlights..."
            ...
            {/* Cash on Delivery (COD) Eligibility Menu */}
            <div>
              <label htmlFor="book-cod-eligibility" className="block text-xs font-semibold text-slate-700 mb-1">
                Cash on Delivery (COD) Eligibility Menu
              </label>
              <div className="relative">
                <select
                  id="book-cod-eligibility"
                  value={cashOnDeliveryEligible ? 'eligible' : 'not_eligible'}
                  onChange={(e) => setCashOnDeliveryEligible(e.target.value === 'eligible')}
                  className="w-full pl-9 pr-4 py-2 text-sm rounded-xl bg-white border border-sky-200 focus:border-sky-500 focus:outline-none focus:ring-2 focus:ring-sky-100 text-slate-900 transition-all cursor-pointer font-medium"
                >
                  <option value="eligible">Eligible for Cash on Delivery (COD)</option>
                  <option value="not_eligible">Not Eligible (Prepaid Only)</option>
                </select>
              ...
              <p className="text-[11px] text-slate-500 mt-1">
                {cashOnDeliveryEligible 
                  ? 'Eligible for both Cash on Delivery (COD) and Online Prepaid in the Books Hub.' 
                  : 'Customer orders for this title must be 100% prepaid online.'}
              </p>
            </div>
        ...
        {/* Right Column: Existing Live Books List with Portrait on Right */}
        <div className="lg:col-span-7 flex flex-col">
          <div className="flex items-center justify-between mb-4 pb-2 border-b border-slate-100">
            <h2 className="text-base font-bold text-slate-900 flex items-center gap-2">
              <BookOpen className="w-4 h-4 text-sky-600" />
              <span>Products in Database ({books.length})</span>
            </h2>
                    ...
                    <div>
                      <div className="flex items-center gap-2 mb-1.5">
                        ...
                        {book.cashOnDeliveryEligible === false ? (
                          ...
                          <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[9px] font-semibold bg-emerald-50 text-emerald-800 border border-emerald-200">
                  ...
                  <div className="w-24 sm:w-28 aspect-[2/3] rounded-xl overflow-hidden bg-slate-100 border border-slate-200 flex-shrink-0 flex items-center justify-center self-center shadow-xs">
                    {book.imageUrl ? (
                      ...
                      <div className="flex flex-col items-center justify-center p-2 text-slate-400 text-center">
                        <BookOpen className="w-6 h-6 stroke-[1.5] text-slate-300" />
            ...
            {books.length === 0 && (
              <div className="text-center py-16 bg-[#F8FAFC] rounded-2xl border border-dashed border-slate-200">
                ...
                <p className="text-xs text-slate-500 mt-1 max-w-xs mx-auto">
                  Use the form on the left to upload your first book product with a portrait cover image.
                </p>
              </div>
            )}
          </div>
