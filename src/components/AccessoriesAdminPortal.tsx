...
export function AccessoriesAdminPortal({ 
  accessories, 
  onBack, 
  onSwitchPortal,
  initialEditAccessory 
}: AccessoriesAdminPortalProps) {
  ...
  // Editing state
  const [editingAccessoryId, setEditingAccessoryId] = useState<string | null>(initialEditAccessory?.id || null);

  // Form State
  const [title, setTitle] = useState(initialEditAccessory?.title || '');
  const [category, setCategory] = useState<string>(initialEditAccessory?.category || ACCESSORY_CATEGORIES[0]);
  const [brand, setBrand] = useState(initialEditAccessory?.brand || '');
  const [price, setPrice] = useState(initialEditAccessory?.price || '');
  const [description, setDescription] = useState(initialEditAccessory?.description || '');
  const [imagePreview, setImagePreview] = useState<string | null>(initialEditAccessory?.imageUrl || null);
  ...
  useEffect(() => {
    if (initialEditAccessory && isAuthenticated) {
      startEditing(initialEditAccessory);
    }
  }, [initialEditAccessory, isAuthenticated]);
  ...
  const startEditing = (item: Accessory) => {
    setEditingAccessoryId(item.id);
    setTitle(item.title);
    setCategory(item.category || ACCESSORY_CATEGORIES[0]);
    setBrand(item.brand || '');
    setPrice(item.price || '');
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (editingAccessoryId) {
        ...
        await updateAccessory(editingAccessoryId, {
          title: title.trim(),
          category,
          brand: brand.trim(),
  ...
  const handleDelete = async (accessoryId: string, accessoryTitle: string) => {
    if (!window.confirm(`Are you sure you want to delete "${accessoryTitle}"?`)) {
      return;
    }
    ...
    try {
      ...
      if (editingAccessoryId === accessoryId) {
        cancelEditing();
      }
      setFeedback({ type: 'success', message: `"${accessoryTitle}" was deleted successfully from Firebase.` });
    } catch (err) {
      console.error('Failed to delete accessory:', err);
      setFeedback({ type: 'error', message: 'Failed to remove product from Firebase.' });
    } finally {
      setDeletingId(null);
    }
  };
      ...
      >
        ...
        <div className="w-full p-8 rounded-3xl bg-[#EAF4FF] border border-[#BFDBFE] shadow-sm flex flex-col items-center text-center">
          ...
          <p className="text-xs text-slate-600 mb-6 max-w-xs leading-relaxed">
            Tech Accessories Portal is password-protected. Enter the administrator passcode to manage inventory and products.
          </p>

          <form onSubmit={handlePasswordSubmit} className="w-full space-y-4 text-left">
            <div>
              <label htmlFor="accessories-admin-passcode" className="block text-xs font-semibold text-slate-700 mb-1.5">
                Master Password
              </label>
              <div className="relative">
                <input
                  id="accessories-admin-passcode"
            ...
            >
              ...
              <span>Unlock Accessories Portal</span>
            </button>
          </form>
    ...
    >
      ...
      <div ref={formTopRef} className="pt-8 pb-4">
        ...
        <p className="mt-1.5 text-sm text-slate-500">
          Upload accessories (Earbuds, Neckbands, Powerbanks, Smartwatches), update pricing, or remove inventory in Firebase.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mt-6">
        {/* Left Column: Direct Upload & Add/Edit Form */}
          ...
          <div className="flex items-center justify-between mb-5 pb-3 border-b border-sky-200/60">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-base font-bold text-slate-900">
                {editingAccessoryId ? 'Edit Accessory' : 'Upload New Accessory'}
              </h2>
            </div>
            {editingAccessoryId && (
              <button
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                className="hidden"
                id="accessory-image-upload-input"
              />
              ...
              {imagePreview ? (
                ...
                >
                  ...
                  <p className="text-xs font-semibold text-slate-800">
                    Upload portrait accessory image
                  </p>
            ...
            {/* Product Name */}
            <div>
              <label htmlFor="acc-title" className="block text-xs font-semibold text-slate-700 mb-1">
              ...
              <input
                ...
                placeholder="e.g. SonicPro ANC Wireless Earbuds"
                required
            ...
            {/* Category: earbuds, neckbands, powerbanks, smartwatch */}
            <div>
              <label htmlFor="acc-category" className="block text-xs font-semibold text-slate-700 mb-1">
            ...
            {/* Brand / Specs */}
            <div>
              <label htmlFor="acc-brand" className="block text-xs font-semibold text-slate-700 mb-1">
                Brand / Highlights
              </label>
              <input
                id="acc-brand"
                ...
                placeholder="e.g. Polarith Audio • 38h Battery • IPX7"
                className="w-full px-3.5 py-2 text-sm rounded-xl bg-white border border-sky-200 focus:border-sky-500 focus:outline-none focus:ring-2 focus:ring-sky-100 text-slate-900 transition-all"
              />
            </div>
            ...
            {/* Price */}
            <div>
              <label htmlFor="acc-price" className="block text-xs font-semibold text-slate-700 mb-1">
                Wholesale Price
              </label>
              <input
                id="acc-price"
            ...
            {/* Description */}
            <div>
              <label htmlFor="acc-desc" className="block text-xs font-semibold text-slate-700 mb-1">
                Description & Packaging / Wholesale Specs
              </label>
              <textarea
                id="acc-desc"
                rows={2}
                ...
                placeholder="Battery capacity, MOQ, packaging specs, warranty..."
            ...
            {/* Payment & Minimum Order Policy Notice */}
            <div className="p-3 rounded-xl bg-amber-50/80 border border-amber-200/80 text-xs text-slate-700 flex items-start gap-2.5">
              <CreditCard className="w-4 h-4 text-amber-700 shrink-0 mt-0.5" />
              <div>
                <p className="font-semibold text-amber-950">Prepaid Only & Min Order ₹4,000</p>
                <p className="text-[11px] text-amber-800 mt-0.5 leading-relaxed">
                  Accessories store orders require a minimum total of ₹4,000. All orders are 100% Prepaid. Cash on Delivery is disabled.
                </p>
              </div>
            </div>
        ...
        {/* Right Column: Existing Live Accessories List with Portrait on Right */}
        <div className="lg:col-span-7 flex flex-col">
          <div className="flex items-center justify-between mb-4 pb-2 border-b border-slate-100">
            <h2 className="text-base font-bold text-slate-900 flex items-center gap-2">
              <Package className="w-4 h-4 text-sky-600" />
              <span>Accessories in Database ({accessories.length})</span>
            </h2>
          ...
          <div className="space-y-3.5 flex-1 overflow-y-auto max-h-[660px] pr-1">
            {accessories.map((item) => {
              const isEditingThis = editingAccessoryId === item.id;
                  ...
                  <div className="w-24 sm:w-28 aspect-[2/3] rounded-xl overflow-hidden bg-slate-100 border border-slate-200 flex-shrink-0 flex items-center justify-center self-center shadow-xs">
                    {item.imageUrl ? (
                      ...
                      <div className="flex flex-col items-center justify-center p-2 text-slate-400 text-center">
                        <Package className="w-6 h-6 stroke-[1.5] text-slate-300" />
            ...
            {accessories.length === 0 && (
              <div className="text-center py-16 bg-[#F8FAFC] rounded-2xl border border-dashed border-slate-200">
                ...
                <p className="text-sm font-bold text-slate-700">No accessories uploaded yet</p>
                <p className="text-xs text-slate-500 mt-1 max-w-xs mx-auto">
                  Use the form on the left to upload earbuds, neckbands, powerbanks, or smartwatches with a portrait image.
                </p>
              </div>
            )}
          </div>
